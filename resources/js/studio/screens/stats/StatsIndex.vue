                                        ...
                                        <div
                                            ...
                                            <div class="mr-auto pl-2">
                                                ...
                                                <p class="text-secondary mb-2">
                                                    {{ post.read_time }} ― {{ trans.app.published }} {{ moment(post.published_at).locale(Studio.locale).fromNow() }}
                                                </p>
                                            </div>
                                            <div class="ml-auto d-none d-lg-block">
                                                <span class="text-muted mr-3">{{ suffixedNumber(post.views_count) }} {{ trans.app.views }}</span>
                                                <span class="mr-3">{{ trans.app.created }} {{ moment(post.created_at).locale(Studio.locale).fromNow() }}</span>
                                            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            fetchStats() {
                this.request()
                    .get(Studio.canvasPath+'/api/stats')
                    .then(response => {
            ...
            fetchPosts($state) {
                this.request()
                    .get(Studio.canvasPath+'/api/posts', {
                        params: {
                            page: this.page,
                            type: 'published'
                        },
