                                    ...
                                    <div class="form-group row mt-3">
                                        <div class="col-12">
                                            <select
                                                ...
                                                name="locale">
                                                <option
                                                    v-for="(locale, code) in Studio.languageCodes"
                                                    :key="code"
                                                    :value="code"
                                                    :selected="Studio.locale === code">
                                                    {{ locale }}
                                                </option>
                                            </select>
                                        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            fetchData() {
                this.request()
                    .get(this.canvasPath+'/api/settings')
                    .then(response => {
            ...
            saveData(data, withNotification) {
                ...
                this.request()
                    .post(this.canvasPath+'/api/settings', data)
                    .then(response => {
                        if (withNotification) {
            ...
            toggleDarkMode() {
                this.$root.Studio.darkMode = this.form.darkMode
                let screen = $('#studio')
            ...
            updateLocale() {
                this.request()
                    .post(this.canvasPath+'/api/locale', { locale: this.form.locale })
                    .then(response => {
                        this.trans = response.data
                        this.$root.Studio.translations = JSON.stringify(response.data)
                        this.$root.Studio.locale = this.form.locale
                    })
                    .catch(error => {
                        //
                    })
