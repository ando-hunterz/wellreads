<template>
  <div class="d-flex flex-column">
    <div v-for="(post, index) in popPosts" :key="post.id" class="row my-4" :style="{ cursor: pointer }" @mouseover="pointer = 'pointer'" @click="redirectPost(post)">
      <div class="popular-rank col-3">
        <h3>0{{index+1}}</h3>
      </div>

      <div class="col side-popular-content">
  ...
  <!-- <div class="row mt-3 pt-3">
      <div class="col text-left">
        <div class="large-popular">
          <h5>
            <Strong>Highlight of The Week in Football!!</Strong>
          </h5>
        </div>
        <div class="popular-paragraph">
          <p class="mb-4">
            Football is a worldwide championship with people all
            around the world participating in. Having such culture is the best of
            humanity.
          </p>
          <p>Raynard Rorong in WellReads</p>
          <p>20 July</p>
        </div>
      </div>
      <div class="col-auto">
        <img class="side-image" src="https://picsum.photos/id/237/197/172" />
      </div>
    </div>
    <div class="row mt-3 pt-3">
      <div class="col text-left">
...
export default {
  ...
  mounted() {
    console.log(this.popPosts)
  },
  methods: {
    redirectPost(post) {
      this.$router.push({
        name: "post",
        params: { identifier: this.publicIdentifier(post), slug: post.slug }
      });
    }
  }
};
